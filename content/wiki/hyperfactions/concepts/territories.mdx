...
The faction map (`/f map`) supports multiple viewing modes and filtering options:

### Terrain Map Mode

In addition to the standard claim overlay, the map supports a **terrain mode** that renders the underlying world terrain alongside faction claims, providing better spatial orientation.

### Map Visibility Filtering

The map supports visibility filtering by relation type, allowing players to show or hide specific categories of claims (e.g., only allied territory, only enemy territory). This helps reduce visual clutter on busy servers with many factions.
