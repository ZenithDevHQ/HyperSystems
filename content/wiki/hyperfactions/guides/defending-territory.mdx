...
- Use the terrain map mode (`/f map`) to scout territory visually

### Map Visibility

HyperFactions supports map visibility filtering per relation type. This means:
- Your faction can control whether enemies can see your claims on the map
- Server admins can configure default visibility settings
- Players with the `hyperfactions.bypass.mapvisibility` permission can see all claims regardless

Use this to your advantage — if your faction is hidden from enemy maps, they'll have a harder time locating and targeting your territory.
...
- [Economy Management](/plugins/hyperfactions/wiki/guides/economy-management) - Treasury for war preparation
